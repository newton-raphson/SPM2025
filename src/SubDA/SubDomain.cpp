...
octToPhysical_(domainExtents.fullDADomain){
  ...
  if(correctCarving != true){
      TALYFEMLIB::PrintWarning("Check carving direction. Atleast one direction must not be carved.");
  }

#endif
}

ibm::Partition SubDomain::functionToRetainPhysNodes(const double *physCoords) const {
...
ibm::Partition SubDomain::classifyNodes(const DENDRITE_REAL * physCoords, ReasonForIn * reasonForIn,const DENDRITE_UINT & nodeID) const{
    ...
    // e. function classifier
    for(const auto & classifiers: this->Voxel::m_inOutClassifier){
      ibm::Partition partition = classifiers(physCoords);
      assert(partition != ibm::Partition::INTERCEPTED);
      if(partition == ibm::Partition::IN){
        reasonForIn[nodeID] = ReasonForIn::FUNCTION;
        return ibm::Partition::IN;
      }
    }
    return ibm::Partition::OUT;
