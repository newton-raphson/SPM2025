...
//    }
    ...
    PetscErrorCode BoundaryConditions::applyVecBC(ot::DA<DIM>* da, Vec rhs) {
        ...
        if(ibmDirichletNodes_!= nullptr) {
            ...
            std::memcpy(&m_boundaryRows[oldBoundaryRowsSize], ibmDirichletNodes_->data(),
                        sizeof(PetscInt) * ibmDirichletNodes_->size());
...
//    }
    ...
    PetscErrorCode BoundaryConditions::fillWithDirichletID_IBM(DA* da,Mat mat){
      ...
      int counter = 0;
      sbmDirichletNodes_.resize(lsz);
      for(PetscInt i = 0; i < lsz; i++){
        if(FEQUALS(array[i],0.0)){
            sbmDirichletNodes_[counter++] = start + i;
        }
      }
      sbmDirichletNodes_.resize(counter);
      sbmDirichletNodes_.shrink_to_fit();
      isAllocatedsbmNodes = true;
      VecRestoreArrayRead(v,&array);
      VecDestroy(&v);
      return 0;
    }

    PetscErrorCode BoundaryConditions::applySBMBoundaryCondition(DA* da, Mat mat){
      /// Do we need to add the vec also???
      if(not isAllocatedsbmNodes){
        fillWithDirichletID_IBM(da,mat);
      }
      if(sbmDirichletNodes_.size() > 0){
//          TALYFEMLIB::PrintWarning("This should not happen unless using untested SBM implementation. Make sure you are doing the right thing");
      }
      for (const auto &row : sbmDirichletNodes_) {
          /// sbmDirichletNodes_ have already accounted for dofs
          MatSetValue(mat, row, row, 1.0, INSERT_VALUES);
      }
