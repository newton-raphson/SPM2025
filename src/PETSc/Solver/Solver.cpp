...
namespace PETSc {
Solver::Solver(DA* da,feMat<DIM> * mat, feVec<DIM> * vec, const DistTREE & distTree, const Point<DIM> & domainMin,const Point<DIM> & domainMax,const DENDRITE_UINT dof,const bool mfree,const bool skipBCApply, const bool oneShotMatrixAssembly):
m_octDA(da),m_Mat(mat),m_Vec(vec),m_oneShotMatrixAssembly(oneShotMatrixAssembly),m_uiDof(dof),m_matrixFree(mfree),m_matCompactRows(da->getNumNodesPerElement(),dof),m_distTree(distTree),m_skipBCApplication(skipBCApply){
