...
void IMGA::addGaussPoints(std::vector<GaussPoint> & gaussPoints, const std::vector<TALYFEMLIB::ZEROPTV> & positions,const std::vector<TALYFEMLIB::ZEROPTV> & normals){
  gaussPoints.resize(positions.size());
  for(int i = 0; i < positions.size(); i++){
    std::memcpy(gaussPoints[i].pos,positions[i].data(),sizeof(double)*DIM);
    std::memcpy(gaussPoints[i].normal,normals[i].data(),sizeof(double)*DIM);
    gaussPoints[i].elemID = i; // Hacks
    gaussPoints[i].geomID = TALYFEMLIB::GetMPIRank();
  }

}
void  IMGA::addPoints(DA * octDA, const std::vector<TALYFEMLIB::ZEROPTV> & points,const std::vector<TALYFEMLIB::ZEROPTV> & normals){
  ...
  if(normals.empty()) {
    ...
    addGaussPoints(gaussPoints, points,normals);
  }
  ...
  // TODO: The communication is sparse. Alltoall not a good communication. May get away with IRecv/ Isend and Iprobe
