...
DA * createSubDA(DistTREE & distTree,const PhysicalDomainDecider & funtion, const DENDRITE_UINT level, const DENDRITE_UINT eleOrder, const double sfcTol){
...
void dendrite_init(int argc, char **argv, int maxDepth) {
...
void printDAStatistics(DA * octDA){
  /**static int callID = 0;
  if(callID == 0 and (TALYFEMLIB::GetMPIRank() == 0)){
    std::ofstream fout("Hist_source");
    std::ofstream fout1("Hist_dest");
    fout.close();
    fout1.close();
  }**/
  int localElemSize = octDA->getLocalElementSz();
  int numSrcNeighbors = octDA->getNumSrcNeighbors();
  int numDestNeighbors = octDA->getNumDestNeighbors();
  int globalInfo[3];
  MPI_Reduce(&localElemSize,&globalInfo[0],1,MPI_INT,MPI_MAX,0,octDA->getCommActive());
  MPI_Reduce(&localElemSize,&globalInfo[1],1,MPI_INT,MPI_MIN,0,octDA->getCommActive());
  MPI_Reduce(&localElemSize,&globalInfo[2],1,MPI_INT,MPI_SUM,0,octDA->getCommActive());
  TALYFEMLIB::PrintStatus("Elements : [Max, Min, Average]",globalInfo[0], " " , globalInfo[1], " " , globalInfo[2]*1.0/(octDA->getNpesActive()));
  int localNodalSize = octDA->getLocalNodalSz();
  MPI_Reduce(&localNodalSize,&globalInfo[0],1,MPI_INT,MPI_MAX,0,octDA->getCommActive());
  MPI_Reduce(&localNodalSize,&globalInfo[1],1,MPI_INT,MPI_MIN,0,octDA->getCommActive());
  MPI_Reduce(&localNodalSize,&globalInfo[2],1,MPI_INT,MPI_SUM,0,octDA->getCommActive());
  TALYFEMLIB::PrintStatus("Nodes : [Max, Min, Average]",globalInfo[0], " " , globalInfo[1], " " , globalInfo[2]*1.0/(octDA->getNpesActive()));

  MPI_Reduce(&numSrcNeighbors,&globalInfo[0],1,MPI_INT,MPI_MAX,0,octDA->getCommActive());
  MPI_Reduce(&numSrcNeighbors,&globalInfo[1],1,MPI_INT,MPI_MIN,0,octDA->getCommActive());
  MPI_Reduce(&numSrcNeighbors,&globalInfo[2],1,MPI_INT,MPI_SUM,0,octDA->getCommActive());
  TALYFEMLIB::PrintStatus("NumSourceNeighbors : [Max, Min, Average]",globalInfo[0], " " , globalInfo[1], " " , globalInfo[2]*1.0/(octDA->getNpesActive()));

  MPI_Reduce(&numDestNeighbors,&globalInfo[0],1,MPI_INT,MPI_MAX,0,octDA->getCommActive());
  MPI_Reduce(&numDestNeighbors,&globalInfo[1],1,MPI_INT,MPI_MIN,0,octDA->getCommActive());
  MPI_Reduce(&numDestNeighbors,&globalInfo[2],1,MPI_INT,MPI_SUM,0,octDA->getCommActive());
  TALYFEMLIB::PrintStatus("NumDestNeighbors : [Max, Min, Average]",globalInfo[0], " " , globalInfo[1], " " , globalInfo[2]*1.0/(octDA->getNpesActive()));
/**
  std::vector<int> histSource(201,0);
  std::vector<int> histDest(201,0);
  if(numDestNeighbors > 200){
    numDestNeighbors = 200;
  }
  if(numSrcNeighbors > 200){
    numSrcNeighbors = 200;
  }

  histDest[numDestNeighbors] = 1;
  histSource[numSrcNeighbors] = 1;
  std::vector<int> globalSource(201,0);
  std::vector<int> globalDest(201,0);
  MPI_Reduce(histSource.data(),globalSource.data(),201,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
  MPI_Reduce(histDest.data(),globalDest.data(),201,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
  if(TALYFEMLIB::GetMPIRank() == 0){
    {
      std::ofstream fout("Hist_source",std::ios::app);
      fout << callID << " ";
      for (const auto &src:globalSource) {
        fout << src << " ";
      }
      fout << "\n";
      fout.close();
    ...
    {
      std::ofstream fout("Hist_dest",std::ios::app);
      fout << callID << " ";
      for (const auto &src:globalDest) {
        fout << src << " ";
      }
      fout << "\n";
      fout.close();
