...
void SubDomainBoundary::generateBoundaryFlags(const Point<DIM> &position, DENDRITE_UINT &id) {
    ...
    if (isInsideFunction(position, id)) {
      this->m_boundary.set(BoundaryTypes::VOXEL::FUNCTION, true);
    }
}
...
bool SubDomainBoundary::isInsideFunction(const Point<DIM> & position, DENDRITE_UINT & id) const {
  for (DENDRITE_UINT i = 0; i < m_subDomain->getFunction().size(); i++) {
    const auto &fun = m_subDomain->getFunction()[i];
    double coords[DIM];
    for (DENDRITE_UINT dim = 0; dim < DIM; dim++) {
      coords[dim] = position.x(dim);
    }
    const auto classifier = fun(coords);
    assert(classifier != ibm::Partition::INTERCEPTED);
    if(classifier == ibm::Partition::IN){
      id = i;
      return true;
    }
  }
