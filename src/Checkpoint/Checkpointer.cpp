...
void Checkpointer::dumpPetscVec(const DA *octDA, std::vector<VecInfo> &vecs, const std::string &filename){
  ...
  for (int i = 0; i < vecs.size(); i++) {
    ...
    std::size_t _lsz = lsz;
    std::size_t _lstart = lstart;
    std::size_t _lend = lend;
    fwrite(&_lsz,    sizeof(size_t), 1, outfile);
    fwrite(&_lstart, sizeof(size_t), 1, outfile);
    fwrite(&_lend,   sizeof(size_t), 1, outfile);
...
void Checkpointer::dumpCellData(const DA * octDA, const std::vector<double> & value,const std::string &filename){
  if (not(octDA->isActive())) {
    return;
  }
  const std::size_t elemSize = octDA->getLocalElementSz();
  const int ndof = std::round(value.size() / (elemSize*1.0));

  FILE *outfile = fopen(filename.c_str(), "w");

  fwrite(&ndof, sizeof(int), 1, outfile);
  fwrite(&elemSize, sizeof(std::size_t ), 1, outfile);
  fwrite(value.data(), sizeof(double),value.size(),outfile);
  fclose(outfile);
}
...
void Checkpointer::readCellData(const DA *octDA, std::vector<double> &value, const std::string &filename) {
  if (not(octDA->isActive())) {
    return;
  }
  ...
  int ndof;
  std::size_t elemSz;
  fread(&ndof, sizeof(int), 1, infile);
  fread(&elemSz,sizeof(std::size_t), 1, infile);
  if(elemSz != octDA->getLocalElementSz()){
    throw std::runtime_error("Mismatch in local element size");
  }
  value.resize(elemSz*ndof);
  fread(value.data(), sizeof(double),elemSz*ndof,infile);
  fclose(infile);
}
...
void Checkpointer::readPetscVecNew(const DA *octDA, std::vector<VecInfo> &vecs, const std::string &filename){
  ...
  for(int i = 0; i < numVecs_; i++){
    ...
    PetscInt lsz(0),lstart(0),lend(0);
    std::size_t flsz(0),flstart(0),flend(0);
    if(octDA->isActive()) {
      VecGetLocalSize(vecs[i].v, &lsz);
      VecGetOwnershipRange(vecs[i].v, &lstart, &lend);
      FILE *infile = fopen(fname.c_str(), "r");

      fread(&flsz, sizeof(std::size_t), 1, infile);
      fread(&flstart, sizeof(std::size_t), 1, infile);
      fread(&flend, sizeof(std::size_t), 1, infile);
      if (infile == NULL)
      {
    //    std::cout << fname << " file open failed " << std::endl;
        return  ;
      }
      if(lsz != (PetscInt)flsz) {
        throw std::runtime_error("Local node Mismatch");
        return;
      }
      if((lstart != (PetscInt)flstart) or (lend != (PetscInt)flend)) {
        throw std::runtime_error(" ownership range Mismatch ");
        return;
      }
...
void Checkpointer::storeCheckpoint(const DA *octDA, const DistTREE *dTree,  std::vector<VecInfo> & vecs,const std::vector<double> & cellData, const DomainExtents & domainExtents,  const TimeInfo *ti) {
  this->storeCheckpoint(octDA,dTree,vecs,domainExtents,ti);
  std::string filename_cellData = folderName_ + "/"  + "cellData" + std::to_string(TALYFEMLIB::GetMPIRank());
  this->dumpCellData(octDA,cellData,filename_cellData);
  MPI_Barrier(MPI_COMM_WORLD);
...
void Checkpointer::loadFromCheckpoint(DA *& octDA, DistTREE & distTree,const PhysicalDomainDecider & domainDecider, std::vector<VecInfo> & vecs, std::vector<double> & cellData, DomainExtents & domainExtents, TimeInfo *ti, const bool vecOnly, const bool doRefine, const double sfc_tol){
  this->loadFromCheckpoint(octDA,distTree,domainDecider,vecs,domainExtents,ti,vecOnly,false,sfc_tol);
  std::string filename_cellData = folderName_ + "/"  + "cellData" + std::to_string(TALYFEMLIB::GetMPIRank());
  this->readCellData(octDA,cellData,filename_cellData);
  MPI_Barrier(MPI_COMM_WORLD);
}
