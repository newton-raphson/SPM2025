...
public:
    NSPNPInputData *inputData_;

    SDARefine(DA *da,  const std::vector<TREENODE> & treePart, const DomainExtents &domainInfo, DENDRITE_UINT maxLevel,NSPNPInputData *inputData);

    ot::OCT_FLAGS::Refine getRefineFlags(TALYFEMLIB::FEMElm &fe, const std::vector<TALYFEMLIB::ZEROPTV> &coords) override;

};

SDARefine::SDARefine(DA *da,  const std::vector<TREENODE> & treePart, const DomainExtents &domainInfo, const DENDRITE_UINT maxLevel,NSPNPInputData *inputData)
