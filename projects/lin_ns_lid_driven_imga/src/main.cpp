...
int main(int argc, char *argv[]) {
    ...
    ////--------------------------------------------------- Vectors---------------------------------------------------////
    Vec prev1Solution, prev2Solution, prev3Solution;
    ...
    ///------------------------------------------Loading geometries ----------------------------------------------///
    GeomInfo geomInfo;
    IMGA *imga = new IMGA(domain, IBM_METHOD::SBM);
...
//        performNoChangeRefinement(octDA, distTree, domain, 5, subDomain, inputData);
    ...
    auto nsEq = InitializeEquations(inputData, talyMesh, octDA, &subDomainBoundary, distTree,
                                    subDomain, ti, imga, kd_trees, boundaryDistanceData);
    ...
    LinearSolver *nsSolver = setLinearSolver(nsEq, octDA, distTree, NSNodeData::NS_DOF, inputData.mfree);
    inputData.solverOptionsNS.apply_to_petsc_options("-");
    KSP nsSolver_ksp = nsSolver->ksp();
    KSPSetFromOptions(nsSolver_ksp);
    if (inputData.ifMMS) {
    ...
    if (!resume_from_checkpoint) {
        if (octDA->isActive()) {
            ...
            octDA->petscCreateVector(prev3Solution, false, false, NSNodeData::NS_DOF);
            setInitialCondition(octDA, inputData, prev1Solution);
        }
    }

#ifdef IBM
    nsEq->setVectors({VecInfo(prev1Solution, NSNodeData::NS_DOF, NSNodeData::VEL_X_PRE1),
                      VecInfo(prev2Solution, NSNodeData::NS_DOF, NSNodeData::VEL_X_PRE2),
                      VecInfo(prev2Solution, NSNodeData::NS_DOF, NSNodeData::VEL_X_PRE3),
    ...
    while (ti.getCurrentTime() < ti.getEndTime() - 1e-15) {
        ...
        ti.print();
        nsSolver->solve();
        VecCopy(prev2Solution, prev3Solution);
        ...
        if (ti.getTimeStepNumber () % inputData.checkpointFrequency == 0) {
            ...
            if (octDA->isActive()) {
                ...
                vecs.emplace_back (VecInfo (prev3Solution, NSNodeData::NS_DOF, NSNodeData::VEL_X_PRE2));
