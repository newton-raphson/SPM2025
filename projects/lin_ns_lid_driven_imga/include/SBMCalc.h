...
void SBMCalc::PtDist2Geo(const TALYFEMLIB::ZEROPTV &pt, double (&d)[DIM])
{
    ...
    size_t num_results = 2;
            ...
            TALYFEMLIB::ZEROPTV OnePointVector,OnePointVectorOtherEnd,OnePointVector2,OnePointVectorOtherEnd2;
...
//            d[1] = m_lines->at(ret_index[0]).lineCoord[0][1] + shift_[1] - y;
            for (int dim =0;dim<DIM;dim++) {
                ...
                OnePointVector2(dim) = m_lines->at(ret_index[1]).lineCoord[0][dim] + shift_[dim] - pt(dim);
                OnePointVectorOtherEnd2(dim) = m_lines->at(ret_index[1]).lineCoord[1][dim] + shift_[dim] - pt(dim);
            }

            TALYFEMLIB::ZEROPTV PickNormalVector= TALYFEMLIB::ZEROPTV{m_lines->at(ret_index[0]).normal[0],m_lines->at(ret_index[0]).normal[1],0.0};
            TALYFEMLIB::ZEROPTV PickNormalVector2= TALYFEMLIB::ZEROPTV{m_lines->at(ret_index[1]).normal[0],m_lines->at(ret_index[1]).normal[1],0.0};
            ...
            TALYFEMLIB::ZEROPTV DistanceVector2 = PickNormalVector2 * scale2;
            for (int dim = 0; dim < DIM; dim++)
            {
                d[dim] = DistanceVector.norm()> DistanceVector2.norm()? DistanceVector2(dim): DistanceVector(dim);
            }

            int PickClosestNumber  = DistanceVector.norm()> DistanceVector2.norm()? 1: 0;

            std::vector<double> Distance2EndPoint = {OnePointVector.norm(),OnePointVectorOtherEnd.norm(),
                                                     OnePointVector2.norm(),OnePointVectorOtherEnd2.norm()};
