...
//    bool inlet = (x < eps) and
//            (y - inputData_->rectLowerLeft[1] < eps) and
//            (y - inputData_->rectUpperRight[1] > eps) and
//            (x - inputData_->rectLowerLeft[0] < eps) and
//            (x - inputData_->rectUpperRight[0] > eps) and
//            boundaries_->checkBoundaryType(BoundaryTypes::VOXEL::FUNCTION);
//
//    bool outlet = (x > eps) and
//                 (y - inputData_->rectLowerLeft[1] < eps) and
//                 (y - inputData_->rectUpperRight[1] > eps) and
//                 (x - inputData_->rectLowerLeft[0] < eps) and
//                 (x - inputData_->rectUpperRight[0] > eps) and
...
//
//
//    bool aerofoil = (y - inputData_->rectLowerLeft[1] > eps) and
//                   (y - inputData_->rectUpperRight[1] < eps) and
//                   (x - inputData_->rectLowerLeft[0] > eps) and
//                   (x - inputData_->rectUpperRight[0] < eps) and
//                   boundaries_->checkBoundaryType(BoundaryTypes::VOXEL::FUNCTION);
...
//            boundaries_->checkBoundaryType(BoundaryTypes::VOXEL::FUNCTION);
    ...
    if (inlet) {
        b.addDirichlet(NSNodeData::VEL_X, 1.0);
        b.addDirichlet(NSNodeData::VEL_Y, 0.0);
    }
    else if (outlet){
        b.addDirichlet(NSNodeData::PRESSURE, 0.0);
