...
struct DeepTraceInputData : public TALYFEMLIB::InputData {

  std::string bf_str;

  DENDRITE_UINT refine_lvl_uni;
  DENDRITE_UINT refine_lvl_bd;
...
//  bool mfree = false;
  ...
  RetainSide stlRetainSide;
    ...
    DeepTraceInputData() : InputData() {}

  ~DeepTraceInputData() = default;
...
//    if (ReadValue("mfree", mfree)) {}
    ...
    if (ReadValue("refine_lvl_uni", refine_lvl_uni)) {}

    if (ReadValue("refine_lvl_bd", refine_lvl_bd)) {}
    ...
    if (!ReadValue("ModelFileName", ModelFileName)) {return false;}
    if (!ReadValue("useDeepLearning", useDeepLearning)) {}
