...
//    std::vector<std::vector<double>> NormalGP;
...
void OptimalSurrogateGpLoop::traverseOperation(TALYFEMLIB::FEMElm & fe, const PetscScalar * values) {
    ...
    z_max_wall = fabs(position.z() - inputData_->physDomain.max[2]) < eps;
...
#endif
                    {
                        //TODO: fix hard code
#if (DIM==3)
                        OnWall = (inputData_->stlRetainSide == true)?false:true;
#endif
                        OnWall = true;
...
//                    std::cout << "OnWall = " << OnWall << "\n";
                    ...
                    if (!OnWall) {
                        ...
                        const double query_pt[3] = {fe.position().x(),fe.position().y(),fe.position().z()}; // shift_
...
//                        }
//                        std::cout << ""
                        std::string normal_fname = "NormalGP.csv";
                        FILE *fp = fopen(normal_fname.c_str(), "a");
                        if (!fp ) {
                            throw TALYException() << "Cannot create file: " << normal_fname;
                        }
                        fprintf(fp,"% .10e, % .10e,",
...
void OptimalSurrogateGpLoop::WriteOptimalGPDistToFile() {
    ...
    if (TALYFEMLIB::GetMPIRank() == 0) {
            ...
            std::string fname2 = "Dist2TrueBoundary.csv";
            std::string normal_fname = "NormalGP.csv";
...
//                        PosGlobal[i].x(),PosGlobal[i].y(),PosGlobal[i].z(),
//                        ,PosDistGlobal[i],NormalGP[i][0],NormalGP[i][1],NormalGP[i][2]);
...
//                        PosGlobal[i].x(),PosGlobal[i].y()
//                        ,PosDistGlobal[i],NormalGP[i][0],NormalGP[i][1]);
#endif
            }
            fclose(fp2);
//            fclose(fp);
