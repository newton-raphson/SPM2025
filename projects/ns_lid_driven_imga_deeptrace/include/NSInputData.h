...
public:
    ...
    enum typeInletBC { LDC = 0 /*default*/, FlowPassingObject = 1, PIPE_FLOW = 2};
    ...
    bool ReadFromFile(const std::string &filename = std::string("config.txt")) {
        ...
        for (DENDRITE_UINT dim = 0; dim < DIM-1; dim++){
    ...
    boundary_def[5].side = BoundaryDef::Side::Z_PLUS;
        ...
        if (InletBCType == FlowPassingObject)
