...
struct OptimalSurrogateStruct {
    std::vector<ZEROPTV> SurrogateGPpos, SurrogateGPnormal, TrueGPnormalGlobal;
...
void performRefinement(DA *& octDA, DistTREE & distTree, DomainExtents &domainInfo,
    ...
    while(true) {
        ...
        if (inputData.InletBCType == NSInputData::LDC) {
            refine = new SDARefine(octDA, distTree.getTreePartFiltered(), domainInfo, maxLevel, &inputData, ti);
        } else {
...
//    SDARefine refine(octDA, distTree.getTreePartFiltered(),domainInfo,maxLevel);
    SDARefine refine(octDA, distTree.getTreePartFiltered(), domainInfo, maxLevel, &inputData,ti);
...
//        std::cout << "elementMarker[i].to_ulong() = " << static_cast<double>(elementMarker[i].to_ulong()) << "\n";
        printMarker[i] = static_cast<double>(/*elementMarker[i].to_ulong()*/checkElement(elementMarker[i]));
    }
...
void refineAndIntergrid(DA *&octDA,
    ...
    while (true) {
        ...
        Refinement *refine;
        if (inputData.InletBCType == NSInputData::LDC) {
            refine = new SDARefine(octDA, dTree.getTreePartFiltered(), domainExtents, maxLevel, &inputData, ti);
        } else {
...
static void perfromNoChangeRefinement(DA *& octDA,DistTREE & dTree, DomainExtents & domainExtents,
    ...
    SDARefine refine(octDA, dTree.getTreePartFiltered(), domainExtents, 8 /*we do not use this number inside refine*/, inputData,ti);
