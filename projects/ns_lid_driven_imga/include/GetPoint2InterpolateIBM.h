...
#ifndef NSSBM_GETPOINT2INTERPOLATEIBM_H
#define NSSBM_GETPOINT2INTERPOLATEIMB_H
...
class GetPoint2InterpolateIBM : public IMGATraversal
{
    ///
...
public:
     ...
     */
    GetPoint2InterpolateIBM(DA *octDA, const IMGA *imga, const std::vector<TREENODE> &treePart, const VecInfo &v, const DomainExtents &domain,
...
GetPoint2InterpolateIBM::GetPoint2InterpolateIBM(DA *octDA, const IMGA *imga, const std::vector<TREENODE> &treePart, const VecInfo &v,
                                           const DomainExtents &domain,
                                           const SubDomain * subDomain, const NSInputData *idata,
                                           std::vector<std::bitset<ElementMarker::MAX_ELMENT_TYPE>> & eleMarkers,SubDomainBoundary *subDomainBoundary)
        : IMGATraversal(octDA, imga, treePart, v, domain)
        ,idata_(idata),subdomain_(subDomain),eleMarkers_(eleMarkers), subDomainBoundary_(subDomainBoundary)
{
...
void GetPoint2InterpolateIBM::ImgaTraversalOperation(const TALYFEMLIB::FEMElm &fe, const NodeAndValues<DENDRITE_REAL> &gaussPoint,
...
void GetPoint2InterpolateIBM::traverseOperation(FEMElm &fe, const PetscScalar *values) {
    ...
    if(hasLocalGaussPoint) {
        while ((gpInfoIt_->localElemID==lclElemID) and (gpInfoIt_ < gpInfoEnd_)) {
            ...
            TALYFEMLIB::ZEROPTV normal = TALYFEMLIB::ZEROPTV{gaussPoint.normal[0], gaussPoint.normal[1], 0};
#if (DIM == 3)
            normal(2) = gaussPoint.normal[2];
#endif
//            std::cout << "normal = "; normal.print(); std::cout <<"\n";

            SurrogateGPposWithSmallMove= fe.position() - normal * ElementSize(fe);
...
void GetPoint2InterpolateIBM::WriteInterGPToFile() {
...
void GetPoint2InterpolateIBM::imgaTraversalOperation(const FEMElm &fe, const NodeAndValues<DENDRITE_REAL> &gaussPoint,
                                                  const ZEROPTV &h, const PetscScalar *values) {

}

void GetPoint2InterpolateIBM::GetpositionOfpointToInterpolateOnAndpositionOnSurface(
...
DENDRITE_REAL GetPoint2InterpolateIBM::ElementSize(const FEMElm &fe) {
