...
#pragma mark Ae-be Surface Domain
                                   ...
                                   const BoundaryDef &bc)
    {
        ...
        //if (idata_->InletBCType == NSInputData::FlowPassingObject) {
...
//            std::cout << "Ae, position = "; fe.position().print();std::cout << "\n";
        ...
        else if (bc.pressure_type == BoundaryDef::Pressure_Type::AVG_PRESSURE) {
            ...
            for (int a = 0; a < nbf; a++)
            {
                for (int b = 0; b < nbf; b++)
                {
                    for (int j = 0; j < nsd; j++)
                    {
                        Ae((nsd + 1) * a + j, (nsd + 1) * b + nsd) +=
                                 fe.N(a) * fe.N(b) * detSideJxW;
                    }
                }
            }
        }
	...
	//if (idata_->InletBCType == NSInputData::FlowPassingObject) {
        ...
        //    if (x_max_wall /*or x_minus_wall*/) {
        //        for (int a = 0; a < nbf; a++) {
        //                for (int j = 0; j < nsd; j++) {
        //                    be((nsd + 1) * a + nsd) +=
        //                            fe.N(a) * u(j) * fe.surface()->normal()(j) * detSideJxW;
        //                }
        //        }
        //    }
        //}
...
//        const int nsd = idata_->nsd;
        ...
        {
////             make sure that we are going inside here as well during calculations
////            otherwise it will be waste of time to do all this calculations
////         std::cout<<"Just making sure we go here"<<std::endl;
            double d[DIM];
            CalculateDistances(fe,CarvedOutGeomID,d);
