...
public:
    ...
    void updateLocation(ZEROPTV locationUpdate) {
        if (refine_type != RegionalRefine::SPHERE) {
            throw TALYFEMLIB::TALYException() << "Have to be a sphere to bind!";
        }
        center_sphere = center_sphere + locationUpdate;
    }
...
struct ParameterDef {
    ...
    /*
     * [ChengHau] default ramping method as LINEAR
     */
    RampingMethod ramping_method = RampingMethod::LINEAR;
...
//        std::cout<<type;
        ...
        switch (type)
        {
            ...
            case Type::CUBE_3D_VOXEL:
                ...
                break;
            default:
                assert(type == CarvedOutGeom::MESHOBJECT || type == CarvedOutGeom::MESHOBJECT_2D || type == CarvedOutGeom::DeepTrace);
        }
...
public:
    ...
    /// This is for multiple BCs
    enum typeInletBC { LDC = 0 /*default*/, FlowPassingObject = 1, PIPE_FLOW = 2, GYROID_FLOW=3};


    enum typeImmersedMethod {ImmersedBoundary = 0, ShiftedBoundary = 1};

    double RatioGPSBM;

    std::vector<RegionalRefine> region_refine;

    OutletBCType outletBcType = OutletBCType::BACKFLOWSTAB;
    ...
    /// ramping
    ParameterDef RampingTerm_Coe;
    bool CoefficientZeroForInitialGuess = false;
    ...
    bool BoundaryTerms = true;
    bool OldNSFormulation = true;
    ...
    typeInletBC InletBCType;

    bool ifMMS = false;
    bool dump_vec = false;
    bool IfScalingStudy = false;
    ...
    double CdStartTimeStep = 0.0;

    int RelativeOrderIntercepted = 5;
    ...
    /// Immersed methods
    typeImmersedMethod ImmersedMethodType;
    ...
    static OutletBCType readOutletBC(libconfig::Setting &root,
                                         const char *name) {
        std::string str;
        /// If nothing specified stays stabilizedNS
        if (root.lookupValue(name, str)) {
            if (str == "BACKFLOWSTAB") {
                return OutletBCType::BACKFLOWSTAB;
            } else if (str == "DONOTHING") {
                return OutletBCType::DONOTHING;
            } else if (str == "PRESSUREOUTLET") {
                return OutletBCType::PRESSUREOUTLET;
            }
            else if (str == "AVG_PRESSURE") {
                return OutletBCType::AVG_PRESSURE;
            }
            else {
                throw TALYFEMLIB::TALYException() << "Unknown time stepping for NS: " << name << str;
            }
        } else {
            PrintStatus("Set Outlet BC type as BackflowStab");
            return BACKFLOWSTAB;
        }
    ...
    static typeImmersedMethod read_ImmersedMethod(libconfig::Setting &root, const char *name) {
        std::string str;
        /// If nothing specified stays stabilizedHT
        if (root.lookupValue(name, str)) {
            if (str == "IBM") {
                return ImmersedBoundary;
            } else if (str == "SBM") {
                return ShiftedBoundary;
            }
        } else {
            throw std::runtime_error("Must specify ImmersedMethod: IBM or SBM");
        }
    }
    ...
    bool ReadFromFile(const std::string &filename = std::string("config.txt")) {
        ...
        RampingTerm_Coe.read_from_config(cfg, "RampingTerm_Coe");

        /// File writing
        if (ReadValue("CdStartTimeStep", CdStartTimeStep)) {}
        if (ReadValue("RelativeOrderIntercepted", RelativeOrderIntercepted)) {}
...
//        if (ReadValue("rect_lvl", rect_lvl)) {}
        ...
        if (cfg.exists("lvl_V")) {
            const libconfig::Setting &lvl_ = cfg.getRoot()["lvl_V"];
            for (int i = 0; i < lvl_.getLength(); ++i) {
                lvl.push_back(lvl_[i]);
            }
        }

        if (cfg.exists("Re_V")) {
            const libconfig::Setting &Re_V_ = cfg.getRoot()["Re_V"];
            for (int i = 0; i < Re_V_.getLength(); ++i) {
                Re_V.push_back(Re_V_[i]);
            }
        }
        ...
        timeStepping = readTimeStepper(cfg.getRoot(), "TimeStepper");
        {
            ...
            if (cfg.exists("OutputSpan_V")) {
                const libconfig::Setting &output_ = cfg.getRoot()["OutputSpan_V"];
                for (int i = 0; i < output_.getLength(); ++i) {
                    OutputSpan_V.push_back(output_[i]);
                }
            } else {
                int output_const;
                ReadValueRequired("OutputSpan", output_const);
                OutputSpan_V.push_back(output_const);
            }
        }
        ...
        if (ReadValue("BoundaryTerms", BoundaryTerms)) {}
        if (ReadValue("OldNSFormulation", OldNSFormulation)) {}
        if (ReadValue("CoefficientZeroForInitialGuess", CoefficientZeroForInitialGuess)) {}



        if (ReadValue("MMS", ifMMS)) {}
        if (ReadValue("dump_vec", dump_vec)) {}
        if(ReadValue("IfScalingStudy",IfScalingStudy))
        {
        }

        solverOptionsNS = read_solver_options(cfg, "solver_options");

        InletBCType = read_InletBC(cfg.getRoot(), "InletBCType");

        outletBcType = readOutletBC(cfg.getRoot(), "OutletBCType");

        if (InletBCType == LDC || InletBCType == FlowPassingObject || InletBCType == PIPE_FLOW || InletBCType == GYROID_FLOW) {
            OutputAverage = false;
        } else {
            OutputAverage = true;
        }

        if (ReadValue("OutputAverage", OutputAverage)) {}
    ...
    boundary_def[5].side = BoundaryDef::Side::Z_PLUS;
        ...
        if (InletBCType == FlowPassingObject and outletBcType == BACKFLOWSTAB)
        { /*set x+ as backflow stabilization*/
            PrintStatus("Setting x+ as backflow stabilization");
            boundary_def[1].pressure_type = BoundaryDef::BACKFLOW_STAB;
        }

        if (InletBCType == FlowPassingObject and outletBcType == AVG_PRESSURE)
        { /*set x+ as pressure average*/
            PrintStatus("Setting x+ as backflow stabilization");
            boundary_def[1].pressure_type = BoundaryDef::AVG_PRESSURE;
        }
        ...
        ImmersedMethodType = read_ImmersedMethod(cfg.getRoot(), "ImmersedMethod");

        if (ImmersedMethodType == ShiftedBoundary) {
            ReadValueRequired("RatioGPSBM", RatioGPSBM);
        }
    ...
    double getReynolds(double t) {
        if (DoReRamping) {
            if (Re_V.empty()) {
//                std::cout << "here\n";
                if (t <= RampingTime) {
//            std::cout << "ramping\n";
                    Re = ((RampingRe - 10) / RampingTime) * t + 10;
                } else {
                    Re = Re_tmp;
                }
            } else {
                Re = Re_V[0];
                for (int idx = totalT.size() - 1; idx > -1; idx--) {
                    if (t >= totalT.at(idx)) {
//                    std::cout << "idx = " << idx << "\n";
                        if (idx == totalT.size() - 1) {
                            Re = Re_V[idx];
                        } else {
                            Re = (Re_V[idx + 1] - Re_V[idx]) * (t - totalT[idx]) / (totalT[idx + 1] - totalT[idx]) +
                                 Re_V[idx];
//                        std::cout << "here \n";
...
private:
    static typeInletBC read_InletBC(libconfig::Setting &root, const char *name) {
        std::string str;
        /// If nothing specified stays stabilizedHT
        if (root.lookupValue(name, str)) {
            if (str == "LDC") {
                PrintStatus("InletBCType is LDC");
                return LDC;
            } else if (str == "FlowPassingObject") {
                PrintStatus("InletBCType is FlowPassingObject");
                return FlowPassingObject;
            } else if (str == "PIPE_FLOW"){
                PrintStatus("InletBCType is PIPE_FLOW");

                return PIPE_FLOW;
            } else if (str == "GYROID_FLOW"){
                PrintStatus("InletBCType is GYROID_FLOW");

                return GYROID_FLOW;
            }
            else {
                PrintError("---------------------------------------");
                PrintError("[Be careful] You try to set InletBCType, but it seems that you have typo or something.");
                PrintError("---------------------------------------");
            }

        } else {
            PrintStatus("---------------------------------------");
            PrintStatus("Default InletBCType is LDC");
            PrintStatus("---------------------------------------");
