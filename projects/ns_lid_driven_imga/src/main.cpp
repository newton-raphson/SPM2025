...
void PrintVectorElements(Vec vec, const char *vecName, PetscInt numElements, const char *fileName) {
    PetscScalar *array;
    PetscInt localSize;

    VecGetArray(vec, &array);
    VecGetLocalSize(vec, &localSize);

    PetscInt numToPrint = PetscMin(localSize, numElements);
    PetscPrintf(PETSC_COMM_WORLD, "Printing first %d elements of %s:\n", numToPrint, vecName);

    // Open file to save the elements
    ...
    for (PetscInt i = 0; i < numToPrint; ++i) {
        PetscPrintf(PETSC_COMM_WORLD, "%s[%d] = %f\n", vecName, i, PetscRealPart(array[i]));
        outFile << vecName << "[" << i << "] = " << PetscRealPart(array[i]) << "\n";
    }
...
//    USING DEEP TRACE FOR GETTING THE MESH ###
    ...
    const auto geomDef = [&](const double *coords) {
        ...
        float dist = 0;
        // this for airplane only for debugging
        floatarr[0] = -1*floatarr[0];
...
//    GeomRefinement ibm_refinements;
//    imga->addGeometry(geomDef, ibm_refinements);
//    GeomInfo geomInfo_ppo;
//    SubDomain subDomain2(domain);
//    IMGA *imga_ppo = new IMGA(domain, (inputData.ImmersedMethodType == NSInputData::ShiftedBoundary) ? IBM_METHOD::SBM
//                                                                                                 : IBM_METHOD::NITSCHE);
    InitializeGeometricObjects(inputData, subDomain, imga, geomInfo);
    ...
    if (!resume_from_checkpoint) {
        int baseLevel=inputData.meshDef.baseLevel;
...
//            bool doRefine = inputData.carved_out_geoms_def.empty()? true : (baseLevel == inputData.carved_out_geoms_def[0].refine_lvl);
//            if (doRefine) {
//                while (true) {
...
//        need to use this for change in the number of processors
...
//    GeomRefinement ibm_refinements(inputData.carved_out_geoms_def[0].geomRefine);
//    InitializeGeometricObjects(inputData, subDomain, imga, geomInfo);
//    //    PrintStatus("The geometry info is",geomInfo.carved_geoms.at(0));
//    imga->addGeometry(geomInfo.carved_geoms.at(0), ibm_refinements);
//    imga->addGeometry(geomDef, ibm_refinements);
//GeomInfo geominfo_ppo;
//
//    InitializeGeometricObjects(inputData, subDomain, imga, geominfo_ppo);
//    imga->addGeometry()
...
//
    ...
    generateNewMarkers(octDA, distTree, domain, subDomain, &subDomainBoundary, imga, elementMarkers_withFalseIntercepted, nodalFalseElement, &inputData);

    Marker *elementMarker_withoutFalseIntercept = new Marker(octDA, distTree.getTreePartFiltered(), domain, imga, MarkerType::GAUSS_POINT);
...
#ifdef IBM
#ifdef DEEPTRACE
    std::vector<my_kd_tree_ptr> kd_trees(1);
    auto nsEq = InitializeEquations(inputData, talyMesh, octDA, &subDomainBoundary, distTree,
                                    subDomain, ti, imga, kd_trees,boundaryDistanceData,queryToIndexMap_vector);
#else
    auto nsEq = InitializeEquations(inputData, talyMesh, octDA, &subDomainBoundary, distTree,
                                    subDomain, ti, imga, kd_trees,boundaryDistanceData,queryToIndexMap_vector);
#endif
...
//    PRINT STATUS
    TALYFEMLIB::PrintStatus("number of geometries:",imga->getGeometries().size());
//    if (!imga->getGeometries().empty()) {
//        TALYFEMLIB::PrintInfo("Do we go inside here not sure");
        optimalSurrogateData =
                PreProcessSBM(inputData, octDA, distTree, nodalFalseElement,
                              ...
                              &ti);
//    }
//    get the number of points in this pointcloud
    TALYFEMLIB::PrintStatus("Point Cloud:",optimalSurrogateData.SurrogateGPpos_PointCloud.kdtree_get_point_count());
//    optimalSurrogateData.SurrogateGPpos_PointCloud.findNearestPoint(Point<double>{0.0,0.0,0.0});

    TALYFEMLIB::PrintStatus("Point Cloud:",optimalSurrogateData.TrueGPpos_PointCloud.kdtree_get_point_count());
...
//            std::cout << "ti.getTimeStepNumber() = " << ti.getTimeStepNumber() << "\n";
        ...
        bool shouldOutput = false
                /*(inputData.InletBCType != NSInputData::FlowPassingObject) ||
                            (ti.getCurrentTime() > inputData.totalT[inputData.totalT.size() - 2])*/;
        // output at the second last time!
        ...
        if (shouldOutput) {
            ...
            for (int i = 0; i < octDA->getLocalNodalSz() * NSNodeData::NS_DOF; i++) {
                ...
                averaged_soln_array[i] = ((SumTimeStep - 1) * averaged_soln_array[i] + soln_array[i]) / SumTimeStep;
            }
...
//            OptimalSurrogateStruct optimalSurrogateData2;
...
//                              NSNodeData::NS_DOF,0);
//
//            VecInfo nodalFalseElementInfo(nodalFalseElement, 1, NSNodeData::NS_DOF);

//            std::vector<VecInfo> vecInfos(2);
//            vecInfos[0] = VecInfo(nsSolver->getCurrentSolution(),
//                                  NSNodeData::NS_DOF,0);
//            vecInfos[1] = VecInfo(nodalFalseElement, 1, NSNodeData::NODE_ID);
//
//            Vec combinedVec;
//            VecInfo combinedVecInfo(combinedVec, NSNodeData::NS_DOF+1, 0);
//
//            PETSc::recombineVec(octDA, vecInfos, combinedVecInfo, false);
//            auto vi = VecInfo(nsSolver->getCurrentSolution(),
//                              NSNodeData::NS_DOF, NSNodeData::VEL_X, PLACEHOLDER_NONE);
//
            auto vi = VecInfo(nsSolver->getCurrentSolution(), NSNodeData::NS_DOF, NSNodeData::VEL_X, PLACEHOLDER_NONE);
#ifdef DEEPTRACE
            VecInfo nodalFalseElementInfo(nodalFalseElement,NSNodeData::NODE_ID,PLACEHOLDER_NONE);
...
//            exit(1);
            ...
            Vec combinedVec;
            interleaveVecs(nsSolver->getCurrentSolution(), nodalFalseElement, &combinedVec, MPI_COMM_WORLD, NSNodeData::NS_DOF, 1);
//
            VecInfo combinedVecInfo(combinedVec, NSNodeData::NUM_VARS+1, PLACEHOLDER_NONE);
// Print the first 100 elements of each vector
            Vec solutionVec = nsSolver->getCurrentSolution();



//            PrintVectorElements(solutionVec, "solutionVec", 284*3,"nsSolutionVec.txt");
//            PrintVectorElements(nodalFalseElement, "nodalFalseElement", 284,"nodalFalseElement.txt");
//            PrintVectorElements(combinedVec, "combinedVec", 284*4,"combinedVec.txt");
//
//            exit(1);
            ...
            ForceCalc force(octDA, distTree.getTreePartFiltered(), combinedVecInfo, domain,
                            elementMarkers_withFalseIntercepted,&subDomain, &subDomainBoundary, &inputData, imga, kd_trees_,ti);

            force.writeTotalForce();
...
//            exit(1);
            ...
            //            DENDRITE_REAL * globalForce;
//            DENDRITE_REAL * projectedArea;

//            force.getTotalForce(globalForce,projectedArea);

//            save the current time step and the total force as well
//
//            TALYFEMLIB::PrintStatus("Force",globalForce);
//            TALYFEMLIB::PrintStatus("ProjectedArea",projectedArea);
#ifndef DEEPTRACE
        ...
        if (ti.getTimeStepNumber() %  ti.getCurrentOutputSpan() == 0) {
            ...
            if (inputData.OutputAverage) {
                ...
                if (shouldOutput) {
                    ...
                    petscVectopvtu(octDA, distTree.getTreePartFiltered(), averaged_soln, folderName, fileName,
                                   varname, domain, false, false, NSNodeData::NS_DOF);
                }
            }
