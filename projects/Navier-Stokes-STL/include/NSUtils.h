...
void performRefinement(DA *&octDA, const DomainExtents &domainInfo, DistTREE &DistTREE, const NSInputData &inputData) {
    while (true) {
        NSRefine refine(octDA, DistTREE.getTreePartFiltered(), domainInfo, DistTREE.getTreePartFiltered().size(),
                        inputData.meshDef.refineLevelBoundary);

        DA *newDA = refine.getRefineSubDA(DistTREE, 0.01, RefinementStrategy::FULL_DOMAIN,
