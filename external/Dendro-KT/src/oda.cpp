...
namespace ot
{
    ...
    DA<dim>::~DA()
    {
      if(m_isNodalAllocated and m_uiIsActive){
        VecDestroy(&m_nodalAllocator);
      }
  ...
  template<unsigned int dim>
  void DA<dim>::getNodes(const DistTree<unsigned int, dim> & distTree) {
    if(m_uiIsActive) {
      if (m_isNodalAllocated) {
        return;
      } else {
        ...
        this->petscCreateVector(m_nodalAllocator,false,false,NODAL_ENTRIES::MAX_ENTRIES);
        VecSet(m_nodalAllocator,0);

        const std::vector<RankI> &ghostedGlobalNodeId = this->getNodeLocalToGlobalMap();
        const size_t ghostedNodalSz = this->getTotalNodalSz();
        const auto odaCoords = this->getTNCoords();
        ...
        while (!treeLoopIn.isFinished()) {
          ...
          if (treeLoopIn.isPre() && subtreeInfo.isLeaf()) {
            ...
            std::vector<double> numEntries(nPe*NODAL_ENTRIES::MAX_ENTRIES,0);

            for (int i = 0; i < nPe; i++) {
              bool isLocal_i = (nodeIdsFlat[i] >= this->getLocalNodeBegin()) and (nodeIdsFlat[i] < (this->getLocalNodeBegin() + this->getLocalNodalSz()));
              for (int j = 0; j < nPe; j++) {
                bool isLocal_j = (nodeIdsFlat[j] >= this->getLocalNodeBegin()) and (nodeIdsFlat[j] < (this->getLocalNodeBegin() + this->getLocalNodalSz()));
                if(i != j) {
                  numEntries[i * NODAL_ENTRIES::MAX_ENTRIES + NODAL_ENTRIES::DIAGONAL] += 1;
                  numEntries[i * NODAL_ENTRIES::MAX_ENTRIES + NODAL_ENTRIES::OFF_DIAGONAL] += 1;
                }
//                if(isLocal_i and isLocal_j) {
//                  numEntries[i * NODAL_ENTRIES::MAX_ENTRIES + NODAL_ENTRIES::DIAGONAL]+=1;
//                }
//                else{
//                  numEntries[i * NODAL_ENTRIES::MAX_ENTRIES + NODAL_ENTRIES::OFF_DIAGONAL]+=1;
//                }
              ...
              PetscInt nodalID[2] = {(PetscInt)nodeIdsFlat[i]*2+0,(PetscInt)nodeIdsFlat[i]*2+1};
              VecSetValues(m_nodalAllocator,2,nodalID,numEntries.data(),ADD_VALUES);
      ...
      VecAssemblyBegin(m_nodalAllocator);
      VecAssemblyEnd(m_nodalAllocator);
      m_isNodalAllocated = true;
...
//          this->getNodes();
          ...
          VecGetArrayRead(m_nodalAllocator,&array);
          std::vector<PetscInt> numEntriesDiagonal(this->getLocalNodalSz()*dof);
          std::vector<PetscInt> numEntriesOffDiagonal(this->getLocalNodalSz()*dof);
          for(size_t i = 0; i < this->getLocalNodalSz(); i++){
            for(int j = 0; j < dof;j++){
              numEntriesDiagonal[i*dof+j] = static_cast<PetscInt>(std::round(array[i*2+0] + 1)*dof);
              numEntriesOffDiagonal[i*dof+j] = static_cast<PetscInt>(std::round(array[i*2+1])*dof);
            }
          }
          VecRestoreArrayRead(m_nodalAllocator,&array);
            ...
            if(isAij || isAijSeq || isAijPrl || isSuperLU || isSuperLU_Dist) {
                if(npesAll > 1) {
                    MatMPIAIJSetPreallocation(M, int{}, numEntriesDiagonal.data(), int{} , numEntriesOffDiagonal.data());
                }else {
                    MatSeqAIJSetPreallocation(M, int{}, numEntriesDiagonal.data());
                }
            }
  ...
  template <unsigned int dim>
  PetscErrorCode DA<dim>::createMatrixWithRows(Mat &M, MatType mtype, const DistTree<unsigned int, dim> & dtree, unsigned int dof) {
    this->getNodes(dtree);
    const unsigned int npesAll=m_uiGlobalNpes;
    const unsigned int eleOrder=m_uiElementOrder;
    const size_t lSz = dof * this->getLocalNodalSz();
    ...
    if(isAij || isAijSeq || isAijPrl || isSuperLU || isSuperLU_Dist) {
      ...
      PetscBool isBAij;
      PetscStrcmp(mtype,MATBAIJ,&isBAij);
      if(isBAij) {
        const PetscScalar * array;
        VecGetArrayRead(m_nodalAllocator,&array);
        std::vector<PetscInt> numEntriesDiagonal(this->getLocalNodalSz()*dof);
        std::vector<PetscInt> numEntriesOffDiagonal(this->getLocalNodalSz()*dof);
        for(size_t i = 0; i < this->getLocalNodalSz(); i++){
            numEntriesDiagonal[i] = std::min(static_cast<PetscInt>(std::round(array[i*2+0] + 1)),(PetscInt)this->getLocalNodalSz());
            numEntriesOffDiagonal[i] = static_cast<PetscInt>(std::round(array[i*2+0] + 1));
        }
        VecRestoreArrayRead(m_nodalAllocator,&array);
        if(npesAll > 1) {
          MatMPIBAIJSetPreallocation(M, dof,int{}, numEntriesDiagonal.data(), int{} , numEntriesOffDiagonal.data());
        }else {
          MatSeqBAIJSetPreallocation(M,dof, int{}, numEntriesDiagonal.data());
        }
