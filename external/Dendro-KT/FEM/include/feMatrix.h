...
{DOLLAR("getAssembledMatrix()")
  ...
  preMat();
  std::vector<PetscInt> blockRows(this->m_uiOctDA->getNumNodesPerElement());
  const int npe = this->m_uiOctDA->getNumNodesPerElement();
  collectMatrixEntries(
           ...
           const std::vector<ScalarT> & colValBuffer )
      {
        ...
        for(int i = 0; i < blockRows.size();i++){
          blockRows[i] = std::round(rowIdxBuffer[i*m_uiDof] / (m_uiDof*1.0));
        }
        {

          MatSetValuesBlocked(*J,npe, blockRows.data(), npe, blockRows.data(), colValBuffer.data(), ADD_VALUES);
        }
      });
  postMat();
