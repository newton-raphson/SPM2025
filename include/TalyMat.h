...
class TalyMat : public feMatrix<TalyMat<Equation, NodeData>, DIM> {
  ...
  std::array<DENDRITE_REAL,DIM> gridDX;
#ifdef TENSOR
...
void TalyMat<Equation, NodeData>::elementalMatVec(const VECType *in, VECType *out, const DENDRITE_UINT ndof, const double *coords,
  ...
  talyElem_->set_elm_id(matVecCommon_.getLocalElemID());
#ifdef IBM
  throw std::runtime_error ("Matrix - free not supported with IBM");
#endif
  memset(out, 0, sizeof(DENDRITE_REAL) * nPe_ * ndof);
  matVecCommon_.convertToPhys(coords,physCoords_,gridDX);
#if TENSOR
  ...
  while (fe_.next_itg_pt()) {
    talyEq_->Integrands_Ae(fe_,Ae_,gridDX.data());
  }
...
void TalyMat<Equation, NodeData>::getElementalMatrix(std::vector<ot::MatRecord> &records, const double *coords, bool isElementalBoundary){
  DOLLAR("ElementalMatrixAssembly")
  ...
  const auto marker = matVecCommon_.getElementMarkerType();
  ...
  if(matVecCommon_.getIBMMethod() == IBM_METHOD::NITSCHE) {
    /// Out element
    if (marker.test(ElementMarker::OUT_GP)) {
      while (fe_.next_itg_pt()) {
        talyEq_->Integrands_Ae(fe_, Ae_,gridDX.data());
      }
    }
    ...
    while (true) {
      if (matVecCommon_.getLocalSurfaceGaussPoint(fe_, gaussPoint)) {
        TALYFEMLIB::ZEROPTV h, position;
        for (int d = 0; d < DIM; d++) {
          h(d) = physCoords_[DIM * (nPe_ - 1) + d] - physCoords_[d];
          position(d) = physCoords_[d];
        }
        talyEq_->ibm_Integrands4side_Ae(fe_, Ae_, gaussPoint, position, h);
      } else {
        break;
      }
    }
      ...
      if (marker.test(ElementMarker::INTERCEPTED_GP)) {

//          std::cout << "matVecCommon_.getInterceptedRelativeOrder() = " << matVecCommon_.getInterceptedRelativeOrder() << "\n";
          fe_.refill(0,matVecCommon_.getInterceptedRelativeOrder());
          while (fe_.next_itg_pt()) {
      ...
      if((marker.test(ElementMarker::SBM_FALSE_INTERCEPTED))
         or (marker.test(ElementMarker::IN_ELEMENT))){
            // Do not do anything on these elements
            // In our terminology, we skip In element
      }
      else{
          // If element is Outside, we do the normal volume assembly
          while (fe_.next_itg_pt()) {
              talyEq_->Integrands_Ae(fe_, Ae_,gridDX.data());
          }
          if((marker.test(ElementMarker::INTERCEPTED_ELEMENT))
             or (marker.test(ElementMarker::SBM_NEIGHBORS_FALSE_INTERCEPTED))){
              // If it is intercepted and not false intercepted
              performSurfaceMatAssembly();
          }
...
void TalyMat<Eqaution, NodeData>::performSurfaceMatAssembly() {
  ...
  for (int i = 0; i < 2*DIM; i++) {
    ...
    if (boundaryType != -1) {
      ...
      while (fe_.next_itg_pt()) {
        talyEq_->Integrands4side_Ae(fe_, boundaryType,boundarySurfaces[i].id, Ae_,gridDX.data());
      }
    }
  }
}
