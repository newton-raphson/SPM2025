...
class Checkpointer{
  ...
  */
  ...
  void dumpCellData(const DA * octDA, const std::vector<double> & value,const std::string &filename);
  void readCellData(const DA * octDA, std::vector<double> & value,const std::string &filename);
 ...
 public:
  ...
  void storeCheckpoint(const DA *octDA, const DistTREE *dTree,  std::vector<VecInfo> & vecs,  const std::vector<double> & cellData, const DomainExtents & domainExtents,const TimeInfo *ti = nullptr) ;

  void loadFromCheckpoint(DA *& octDA, DistTREE & distTree,const PhysicalDomainDecider & domainDecider, std::vector<VecInfo> & vecs,  DomainExtents & domainExtents,TimeInfo *ti = nullptr, const bool vecOnly=false, const bool doRefine = false, const double sfc_tol = 0.001);
  void loadFromCheckpoint(DA *& octDA, DistTREE & distTree,const PhysicalDomainDecider & domainDecider, std::vector<VecInfo> & vecs, std::vector<double> & cellData, DomainExtents & domainExtents, TimeInfo *ti = nullptr, const bool vecOnly=false, const bool doRefine = false, const double sfc_tol = 0.001);
