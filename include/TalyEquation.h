 ...
 */
  ...
  inline void setRelativeOrder(const int * relativeOrder){
    ...
    inline void setInterceptedRelativeOrder(const int * relativeOrder){
        matVecCommon.assignInterceptedRelativeOrder(relativeOrder);
    }
  ...
  void assignIBMConstructs(const IMGA *imga, const std::bitset<ElementMarker::MAX_ELMENT_TYPE>  * marker, int sbmNodeID = -1){
    matVecCommon.assignIMGAConstructs(imga,marker);

    if(imga->getIBMMethod() == IBM_METHOD::SBM){
        assert(sbmNodeID != -1);
        matVecCommon.assignNodeID(sbmNodeID);
