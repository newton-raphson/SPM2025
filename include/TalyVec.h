...
void TalyVec<Equation,NodeData>::elementalComputeVec(const VECType *in,
  ...
  auto marker = matVecCommon_.getElementMarkerType();
  if(matVecCommon_.getIBMMethod() == NITSCHE) {
      bool doGlobalAssemble = false;
      if (marker.test(ElementMarker::OUT_GP)) {
          while (fe_.next_itg_pt()) {
              talyEq_->Integrands_be(fe_, be_,gridDX.data());
          }
      }
      ...
      if((marker.test(ElementMarker::SBM_FALSE_INTERCEPTED))
         or (marker.test(ElementMarker::IN_ELEMENT))){
          ...
          // If element is Inside, we do the normal volume assembly
          while (fe_.next_itg_pt()) {
              talyEq_->Integrands_be(fe_, be_,gridDX.data());
          }
...
void TalyVec<Eqaution, NodeData>::performSurfaceVecAssembly() {
  ...
  for (int i = 0; i < 2*DIM; i++) {
    ...
    if (boundaryType != -1) {
      ...
      while (fe_.next_itg_pt()) {
        talyEq_->Integrands4side_be(fe_, boundaryType,boundarySurfaces[i].id, be_,gridDX.data());
      }
    }
  }
}
