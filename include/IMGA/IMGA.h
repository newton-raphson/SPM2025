...
class IMGA {
  ...
  void addGaussPoints(std::vector<GaussPoint> &gaussPoints, const std::vector<TALYFEMLIB::ZEROPTV> & positions,const std::vector<TALYFEMLIB::ZEROPTV> & normals);
 ...
 public:
  ...
  void addPoints(DA * octDA,const std::vector<TALYFEMLIB::ZEROPTV> & points, const std::vector<TALYFEMLIB::ZEROPTV> & normals);
  ...
  void inline initIMGAComputation(DA * octDA, const std::vector<TREENODE> & treePart, const std::vector<TALYFEMLIB::ZEROPTV> & position) {
    ...
    std::vector<TALYFEMLIB::ZEROPTV> normals;
    addPoints(octDA,position,normals);
    findBackGroundElements(octDA, treePart);
  }
  void inline initIMGAComputation(DA * octDA, const std::vector<TREENODE> & treePart,
                                  const std::vector<TALYFEMLIB::ZEROPTV> & positionOfPointToInterpolate,
                                  const std::vector<TALYFEMLIB::ZEROPTV> & surfacePosition) {
        addPoints(octDA,positionOfPointToInterpolate,surfacePosition);
        findBackGroundElements(octDA, treePart);
    }
  void inline initIMGAComputation(DA * octDA, const std::vector<TREENODE> & treePart) {
  ...
  template<int dof>
  void findBackGroundElements(const DA * octDA, std::vector<SurfaceValues<dof>> & surfaceValues, const std::vector<TREENODE> & treePart){
    ...
    // Sort
    {
      ...
      std::sort(permutationIndex.begin(), permutationIndex.end(),
                [&](int i, int j) { return (m_gpInfo[i] < m_gpInfo[j]); });
      std::transform(permutationIndex.begin(), permutationIndex.end(), m_gpInfo.begin(),
                     [&](int i) { return m_gpInfo[i]; });
      std::transform(permutationIndex.begin(), permutationIndex.end(), surfaceValues.begin(),
  ...
  template<int dof>
  inline void translateGaussPoints(DA * octDA, std::vector<SurfaceValues<dof>> & surfaceValues, const TALYFEMLIB::ZEROPTV & translation) {
    ...
    assert(movedSurfaceValues.size()==gaussPoints_.size());
    // Now at this moment, Gauss point and surface values are at moved according to the new position.

    this->findBackGroundElements(octDA,surfaceValues,treeNodes);
