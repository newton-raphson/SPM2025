...
namespace PETSc {
  ...
  LinearSolver *setLinearSolver(TalyEquation<Equation, NodeData> *talyEq,
                                ot::DA<DIM> *da,const DistTREE & distTree,
                                DENDRITE_UINT ndof = 1,
                                bool mfree = true,const bool skipBCAssembly = false,
                                const bool oneShotAssembly = false) {

    LinearSolver *solver = new LinearSolver(da, talyEq->getMat(), talyEq->getVec(), distTree,talyEq->getDomainMin(),
                                            talyEq->getDomainMax(), ndof, mfree,skipBCAssembly, oneShotAssembly);
    if (da->isActive()) {
      solver->init();
    }
    return solver;
  ...
  NonlinearSolver *setNonLinearSolver(TalyEquation<Equation, NodeData> *talyEq,
    ...
    NonlinearSolver *solver = new NonlinearSolver(da, talyEq->getMat(), talyEq->getVec(), distTree,talyEq->getDomainMin(),
                                                  talyEq->getDomainMax(), ndof, mfree,skipBCAssembly, oneShotAssembly);
    if (da->isActive()) {
      solver->init();
    }
  ...
  static void recombineVec(DA *da, std::vector<VecInfo> &inVecs, std::vector<PetscScalar> & combinedVec) {
    DENDRITE_UINT ndof = 0;
    for (auto &vec: inVecs) {
      ndof += vec.ndof;
      VecGetArrayRead(vec.v, &vec.val);
    ...
    da->createVector(combinedVec,false,false,ndof);
    ...
    for (DENDRITE_UINT node = 0; node < localNodalSz; node++) {
      currentDof = 0;
      for (const auto &vec: inVecs) {
        std::memcpy(&combinedVec[node * ndof + currentDof], &vec.val[node * vec.ndof], sizeof(PetscScalar) * vec.ndof);
        currentDof += vec.ndof;
      }
    }
  ...
  static void scatterVec(DA *octDA, std::vector<VecInfo> &outVec, const std::vector<PetscScalar>& combinedVec,bool isAllocated = false) {
    int ndof = 0;
    for (auto &vec:outVec) {
      ndof += vec.ndof;
    }
    ...
    if (octDA->isActive()) {
      ...
      const int localNodalSz = octDA->getLocalNodalSz();
      const int totalDof = ndof;
      for (int i = 0; i < localNodalSz; i++) {
        int currentDofIndex = 0;
        for (int j = 0; j < outVec.size(); j++) {
          std::memcpy(&arrayWrite[j][i * outVec[j].ndof], &combinedVec[i * totalDof + currentDofIndex],
                      sizeof(PetscScalar) * outVec[j].ndof);
          currentDofIndex += outVec[j].ndof;
        }
      }
 ...
 */
  ...
  static void updateEquationAndSolver(DA *newDA, const DistTREE & distTree,
    ...
    talyEquation = new TalyEquation<Equation, NodeData>(newDA, distTree.getTreePartFiltered(), domainExtents, ndof, timeInfo, m_surfAssembly,
                                                        subDomainBoundary, args...);
    bool mfree = solver->get_mfree();
    bool oneShotAssembly = solver->oneShotAssembly();
    bool skipBCAssembly = solver->ifskipBCApply();
    const auto bnd = solver->getBoundaryCondition();
    delete solver;
    solver = new LinearSolver(newDA, talyEquation->getMat(), talyEquation->getVec(), distTree,talyEquation->getDomainMin(),
                              talyEquation->getDomainMax(), ndof, mfree,skipBCAssembly, oneShotAssembly);
  ...
  static void updateEquationAndSolver(DA *newDA, const DistTREE & distTree,
    ...
    bool skipBCAssembly = solver->ifskipBCApply();
    delete solver;
    solver = new NonlinearSolver(newDA, talyEquation->getMat(), talyEquation->getVec(), distTree, talyEquation->getDomainMin(),
                                 talyEquation->getDomainMax(), ndof, mfree, skipBCAssembly,oneShotAssembly);
    solver->setDirichletBoundaryCondition(bnd);
    if (newDA->isActive()) {
      solver->init();
  ...
  template<typename Equation, typename NodeData, typename ... Args>
  static void updateEquationAndSolver(TalyMesh<NodeData> *mesh, DA *newDA,const DistTREE & distTree,
                                      TalyEquation<Equation, NodeData> *&talyEquation,
                                      const DomainExtents &domainExtents,
                                      LinearSolver *&solver,
                                      const TimeInfo *timeInfo = nullptr,
    ...
    bool m_surfAssembly = talyEquation->performSurfAssembly();
    delete talyEquation;
    talyEquation = new TalyEquation<Equation, NodeData>(mesh, newDA, distTree.getTreePartFiltered(), domainExtents, ndof, timeInfo,
  ...
  static void updateEquationAndSolver(TalyMesh<NodeData> *mesh, DA *newDA, const DistTREE & distTree,
    ...
    template<typename Equation, typename NodeData, typename ... Args>
    static void updateSolver(TalyMesh<NodeData> *mesh, DA *newDA, const DistTREE & distTree,
                                        TalyEquation<Equation, NodeData> *&talyEquation,
                                        const DomainExtents &domainExtents,
                                        NonlinearSolver *&solver,
                                        const TimeInfo *timeInfo = nullptr,
