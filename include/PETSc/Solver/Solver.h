...
class Solver {
 ...
 protected:
  ...
  const bool m_skipBCApplication;

 public:

  Solver(DA* da,feMat<DIM> * mat, feVec<DIM> * vec,const DistTREE & distTree, const Point<DIM> & domainMin,const Point<DIM> & domainMax, const DENDRITE_UINT dof,  const bool mfree, const bool skipBCApply, const bool oneShotMatrixAssembly);
  ...
  auto getMat()const{
    return m_matJacobian;
  }

  bool ifskipBCApply(){
    return m_skipBCApplication;
  }
