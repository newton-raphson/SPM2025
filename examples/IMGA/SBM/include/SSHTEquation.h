...
class SSHTEquation : public TALYFEMLIB::CEquation<SSHTNodeData> {
 ...
 public:
  ...
  void Integrands4side_Ae(const TALYFEMLIB::FEMElm &fe, const DENDRITE_UINT side_idx, const DENDRITE_UINT id, TALYFEMLIB::ZeroMatrix<double> &Ae, const double *h) override {
...
//        Ae(a,b) += alpha/h[0]*(fe.N(a) + gradWdotd + secondOrderTerm_a)*(gradUdotd + fe.N(b) + secondOrderTerm_b)*fe.detJxW();
//      }
//
//    }
  }

  void Integrands4side_be(const TALYFEMLIB::FEMElm &fe, const DENDRITE_UINT side_idx, const DENDRITE_UINT id, TALYFEMLIB::ZEROARRAY<double> &be, const double  *h) override {
      std::ofstream fout("GP_"+std::to_string(TALYFEMLIB::GetMPIRank())+".txt",std::ios::app);
